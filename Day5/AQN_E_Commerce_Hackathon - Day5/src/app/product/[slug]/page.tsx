...
export default function ProductPage({ params }: ProductPageProps) {
  ...
  if (!product) return <p>Loading...</p>; // Show a loading message until data is fetched
    ...
    <div className="max-w-7xl mx-auto px-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div className="flex flex-col gap-8">
          ...
          <p className="text-gray-500 mt-1 text-right">{product.category}</p>
          <p className="text-left text-2xl">{product.colors}</p>
          <p className="mt-1 font-semibold">Stocks: {product.inventory} pcs</p>
          <p className="text-center mb-1 bg-yellow-400">{product.status}</p>
          <button
            className="bg-gradient-to-r from-red-600 to-red-800 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:shadow-lg hover:scale-110 transition-transform duration-300 ease-in-out"
            onClick={(e) => handleAddToCart(e, product)}
          >
            Add To Cart
          </button>
